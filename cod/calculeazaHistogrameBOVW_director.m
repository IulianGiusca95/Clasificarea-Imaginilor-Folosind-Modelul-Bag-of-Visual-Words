...
%       histogrameBOVW - matrice #Imagini x K, fiecare linie reprezinta histograma BOVW a unei imagini
  ...
  for i=1:numarImagini 
    ...
    % completati codul
    puncteC=genereazaPuncteCaroiaj(img,nrPuncteX,nrPuncteY,margine);
    % calculeaza descriptorul HOG pentru fiecare punct
    % completati codul
    [descriptoriHOG,p]=calculeazaHistogrameGradientiOrientati(img,puncteC,dimensiuneCelula);
    % calculeaza histograma BOW asociata    
    % completati codul
    histograma = calculeazaHistogramaBOVW(descriptoriHOG,cuvinteVizuale);
    histogrameBOVW(i,:)=histograma;
  end
  %disp(histogrameBOVW);
